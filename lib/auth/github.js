...
exports.authenticateUser = function(req, res, next) {
  ...
  if (_.has(req.query, "cb")) {
    console.log(
      "[services.github] - Callback workflow detected, attempting to process data..."
    );
    console.log("------------------------------------------------------------");

    passport.authenticate("github", { session: false }, function(
      err,
      data,
      info
    ) {
      if (err || !data) {
        console.log(
          "[services.github] - Error retrieving GitHub account data - " +
            JSON.stringify(err)
        );
      ...
      console.log(
        "[services.github] - Successfully retrieved GitHub account data, processing..."
      );
      console.log(
        "------------------------------------------------------------"
      );
  ...
  request(
    ...
    function(err, data) {
      if (err) {
        console.log(err);
        console.log(
          "[services.github] - Error retrieving GitHub email addresses."
        );
        console.log(
          "------------------------------------------------------------"
        );
